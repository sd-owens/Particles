...
Animate( )
{
	...
	if( ShowPerformance )
	{
		...
		double GigaTrialsPerSec = (double) NUM_PARTICLES/ElapsedTime/1000000000.;

		if (GigaTrialsPerSec > maxPerformance)
			maxPerformance = GigaTrialsPerSec;
	}
...
Display( )
{
	...
	{
		char str[128];
		sprintf( str, "%6.3f GigaParticles/Sec", (float)NUM_PARTICLES/ElapsedTime/1000000000. );
...
InitGlui( )
{
	...
	//Glui = GLUI_Master.create_glui_subwindow(MainWindow, GLUI_SUBWINDOW_TOP);
...
Reset( )
{
	...
	Paused = GLUIFALSE;
	Scale  = 1.0;
	Scale2 = 0.;		// because add 1. to it in Display( )
...
ResetParticles( )
{
	...
	for( int i = 0; i < NUM_PARTICLES; i++ )
	{
		colors[i].r = .1; // Ranf( .3f, 1. );
		colors[i].g = .1; // Ranf( .3f, 1. );
		colors[i].b = .9; // Ranf( .3f, 1. );
		colors[i].a = 1.;
	}
	glUnmapBuffer( GL_ARRAY_BUFFER );
...
Quit( )
{
	...
	fprintf(stderr, "\nNumParticles: %d\tMax Performance: %6.8lf GigaParticlesPerSec\n", NUM_PARTICLES, maxPerformance);
